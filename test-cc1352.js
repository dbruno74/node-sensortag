...
SensorTag.discover(function(sensorTag) {
  ...
  async.series([
      ...
      function(callback) {
	console.log('\tLED 0: ON');
	sensorTag.writeLed(0,1,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('\tLED 0: OFF');
	sensorTag.writeLed(0,0,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('\tLED 1: ON');
	sensorTag.writeLed(1,1,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('\tLED 1: OFF');
	sensorTag.writeLed(1,0,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('\tLED 2: ON');
	sensorTag.writeLed(2,1,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('\tLED 2: OFF');
	sensorTag.writeLed(2,0,function(error) {
          setTimeout(function(){callback();}, 2000);
        });
      },
      function(callback) {
	console.log('readBatteryLevel');
	sensorTag.readBatteryLevel(function(error,perc) {
	    console.log('\tError: ' +error);
	    console.log('\tBattery Level = ' +perc.toFixed(1) +'%');
	    callback();
        });
        sensorTag.on('batteryLevelChange', function(perc) {
            console.log('\tBattery Level = ' +perc.toFixed(1) +'%');
        });
        console.log('enableBatteryLevel');
      ...
      function(callback) {
	if (sensorTag.type === 'cc1352') {
          console.log('button_0Change - waiting for button press ...');
          sensorTag.on('button_0Change', function(key) {
            console.log('left: ' + key);

         //   if (key) {
         //     sensorTag.notifyButton(callback);
         //   }
          });

          console.log('button_1Change - waiting for button press ...');
          sensorTag.on('button_1Change', function(key) {
